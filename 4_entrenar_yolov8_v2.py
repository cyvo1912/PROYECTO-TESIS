...
# ============================================================
# SCRIPT 4 V2 - ENTRENAMIENTO YOLOV8S CON LOSO (8 SUJETOS)
# ============================================================
...
DATASET_RAIZ  = r"D:\PROYECTO SEMINARIO VENEGAS NO BORRAR\dataset_final_v2"
DATASET_LOSO  = r"D:\PROYECTO SEMINARIO VENEGAS NO BORRAR\dataset_loso_v2"
...
PROYECTO      = "resultados_v2_loso"
NOMBRE_BASE   = "yolov8s_loso"
...
SUJETOS_TEST = ["GRABACION 1", "GRABACION 5"]    # Mujer + Hombre
SUJETOS_CV   = ["GRABACION 2", "GRABACION 3", "GRABACION 4",
                "GRABACION 6", "GRABACION 7", "GRABACION 8"]
N_FOLDS      = len(SUJETOS_CV)                    # 6 folds
...
colores_fold = ["#2196F3", "#4CAF50", "#FF9800", "#9C27B0", "#F44336", "#00BCD4"]
...
# ============================================================
# PASO 2 - PREPARAR TEST SET (2 sujetos)
# ============================================================

def preparar_test():

    print(f"\n  Preparando Test set con sujetos: {SUJETOS_TEST}")

    for clase in CLASES:
        os.makedirs(os.path.join(DATASET_LOSO, "test", clase), exist_ok=True)
        total = 0
        for sujeto in SUJETOS_TEST:
            imagenes = obtener_imagenes_sujeto(clase, sujeto)
            for img in imagenes:
                shutil.copy(str(img), os.path.join(DATASET_LOSO, "test", clase, img.name))
            total += len(imagenes)
        print(f"    {clase:<25s}: {total:>4} imágenes")
...
if __name__ == "__main__":
    ...
    print("\n" + "="*60)
    print("  YOLOV8 V2 - LOSO (8 SUJETOS)")
    print(f"  Test: {SUJETOS_TEST}")
    print(f"  CV:   {SUJETOS_CV} ({N_FOLDS} folds)")
    print(f"  {MODELO_BASE} | {EPOCHS} épocas | {OPTIMIZER}")
    print(f"  LR: {LR0} | Dropout: {DROPOUT} | Batch: {BATCH_SIZE}")
    print("="*60)
    ...
    print("\n" + "="*60)
    print("  RESUMEN CROSS-VALIDATION V2 (por sujeto)")
    print("="*60)
    ...
    # ========================================================
    # EVALUACIÓN FINAL SOBRE TEST (2 sujetos)
    # ========================================================

    print("\n" + "="*60)
    print(f"  EVALUACIÓN FINAL SOBRE TEST: {SUJETOS_TEST}")
    print("="*60)
    ...
    ruta_txt = os.path.join(RESULTADOS_DIR, "reporte_loso_v2.txt")
    with open(ruta_txt, "w", encoding="utf-8") as f:
        f.write("REPORTE LOSO V2 - YOLOv8s Distracciones (8 sujetos)\n")
        f.write("="*60 + "\n")
        f.write(f"Modelo: {MODELO_BASE} | Épocas: {EPOCHS} | LR: {LR0}\n")
        f.write(f"Dropout: {DROPOUT} | Optimizer: {OPTIMIZER}\n")
        f.write(f"Sujetos Test: {SUJETOS_TEST}\n")
        f.write(f"Sujetos CV: {SUJETOS_CV}\n")
        f.write("="*60 + "\n\n")
        ...
        f.write(f"\nEVALUACIÓN FINAL SOBRE TEST ({SUJETOS_TEST}):\n")
        f.write("="*60 + "\n")
        f.write(reporte)
    ...
    ax.set_title("Métricas por Fold - LOSO V2 (8 sujetos)", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    ruta_graf = os.path.join(RESULTADOS_DIR, "metricas_por_fold_loso_v2.png")
    ...
    fig.suptitle(
        f"Matriz de Confusión V2 - Test: {SUJETOS_TEST}",
    ...
    plt.tight_layout()
    ruta_cm = os.path.join(RESULTADOS_DIR, "matriz_confusion_test_v2.png")
    ...
    if csvs:
        ...
        fig.suptitle("Comparación de Folds LOSO V2 - Loss y Accuracy", fontsize=14, fontweight="bold")
        ...
        plt.tight_layout()
        ruta_comp = os.path.join(RESULTADOS_DIR, "comparacion_folds_loso_v2.png")
    ...
    ax.set_title(f"F1-Score por Clase V2 - Test ({', '.join(SUJETOS_TEST)})", fontsize=13, fontweight="bold")
    ...
    plt.tight_layout()
    ruta_f1 = os.path.join(RESULTADOS_DIR, "f1_por_clase_test_v2.png")
    plt.savefig(ruta_f1, dpi=150, bbox_inches="tight")
    plt.close()
    print(f"  [OK] F1 por clase guardado en: {ruta_f1}")

    print(f"\n{'='*60}")
    print(f"  LOSO V2 COMPLETADO")
    print(f"  Resultados en: {RESULTADOS_DIR}")
    print(f"{'='*60}")
    print(f"""
  ARCHIVOS GENERADOS:
    {RESULTADOS_DIR}/
      reporte_loso_v2.txt              -> CV + Test final
      metricas_por_fold_loso_v2.png    -> Barras por fold
      matriz_confusion_test_v2.png     -> Confusión del test
      comparacion_folds_loso_v2.png    -> Loss y Accuracy superpuestos
      f1_por_clase_test_v2.png         -> F1 por clase del test
    """)
